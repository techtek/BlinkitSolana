...
:: Let the user know that Blinkit is going to start and watch for new blockchain action with these details:
	echo %Blue%Serum %Grey%Blink on SRM price below the set alert price%Grey%
	echo.
...
:: Let the user know the selected serumnode	  
...
:: Let the user know that the program is starting to look for new actions
	echo.
	echo %White%Blinkit is now connecting your USB flash drive to the %Blue%Serum%White% Price...	  
	echo.
...
:: Deletes unwanted characters from the srmpricestriped.txt and keep only the numeric value
	...
	FOR /f "delims=" %%i IN (%~dp0\data\srmpricestriped.txt) DO (
	SET line=%%i
	SET line=!line:"=!
	SET line=!line:serum=!
	SET line=!line:usd=!
	...
	SET line=!line:}=!
	SET line=!line: =!
	echo !line! > %~dp0\data\srmpricestriped.txt
	)	
...
:: Deletes unwanted characters from the srmpricestriped2.txt and keep only the numeric value
	...
	FOR /f "delims=" %%i IN (%~dp0\data\srmpricestriped2.txt) DO (
	SET line=%%i
	SET line=!line:"=!
	SET line=!line:serum=!
	SET line=!line:usd=!
	...
	SET line=!line:}=!
	SET line=!line: =!
	echo !line! > %~dp0\data\srmpricestriped2.txt
	)
